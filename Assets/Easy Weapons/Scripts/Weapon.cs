	...
	void CheckForUserInput()
	{
		...
		if (type == WeaponType.Raycast)
		{
			if (fireTimer >= actualROF && burstCounter < burstRate && canFire)
			{
				...
				if (warmup && CrossPlatformInputManager.GetButtonUp("Jump"))
				{
		...
		if (Input.GetButtonDown("Reload") || CrossPlatformInputManager.GetButtonDown("Reload"))
			Reload();
	...
	void Fire()
	{
		...
		for (int i = 0; i < shotPerRound; i++)
		{
			...
			if (Physics.Raycast(ray, out hit, range))
			{
                ...
                {
                    hit.collider.gameObject.SendMessageUpwards("ChangeHealth", -damage * 10, SendMessageOptions.DontRequireReceiver);
                }
                else
                {
